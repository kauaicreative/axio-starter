...
/**
 * Add debug to body class if debug GET variable exists
 */
add_filter( 'body_class', function( $classes ) {
	if ( isset( $_GET['debug'] ) ) $classes[] = 'debug';
...
if ( ! class_exists( 'SD_Debug' ) ) {
	...
	if ( ! function_exists( 'd' ) ) {
		...
		function d( $obj = '', $args = [ 'print' => true ] ) {
			if ( false && class_exists( 'QM_Plugin' ) ) {
				do_action( 'qm/debug', $obj );
			}
			elseif ( WP_DEBUG ) {
				?>
				<div style="position:sticky; z-index: 9999999999999; background:rgba(230,230,230,0.94);color:#000; padding:40px;">
					<?php
					...
					$obj = '<script>document.getElementsByTagName("code").getElementsByTagName("span").remove() ;document.getElementsByTagName("code")[0].getElementsByTagName("span")[document.getElementsByTagName("code")[0].getElementsByTagName("span").length - 1].remove() ; </script>';
					?>
				</div>
				<?php
