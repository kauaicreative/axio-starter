...
const cssTasks = (filename) => {
	return lazypipe()
		...
		.pipe(function() {
			return gulpif('*.scss',
				//⚠️ Note: With Dart Sass, synchronous rendering is twice as fast as asynchronous rendering. The Sass team is
				// exploring ways to improve asynchronous rendering with Dart Sass, but for now you will get the best
				// performance from sass.sync()
				// https://www.npmjs.com/package/gulp-sass#migrating-to-version-5
				sass.sync({
					//outputStyle:     'compressed',//'nested', // libsass doesn't support expanded yet
					//precision:       8,
					includePaths:    ['.'],
